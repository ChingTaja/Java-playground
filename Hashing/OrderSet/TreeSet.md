...
headSet、tailSet、subSet 產生的新集合都是 Backed Set（底層連動的視圖）


|方法| 運作範圍|預設行為（未傳 boolean） | 關聯與背後邏輯     |         |
| -------------- | ---------------- | ---------------- | ----------------------- | ------- |
| `headSet(E)`   | 截取頭部（小於 E 的元素）   | 不包含 E（Exclusive） | 取得所有「排在 E 之前」的元素        |         |
| `tailSet(E)`   | 截取尾部（大於等於 E 的元素） | 包含 E（Inclusive）  | 從 E 開始一路到集合最後           |         |
| `subSet(A, B)` | 截取 A 到 B 之間      | 包含 A，不包含 B       | 對應數學區間 **[A, B)**（左閉右開） | **]()** |

簡單來說：它不是「複製（Copy）」一份新的資料給你，而是回傳一個基於原始數據源的視圖（View）, 他們共享相同的數據源底層


### 什麼時候該用 TreeSet？（與 HashSet / ArrayList）


既然 HashSet 速度最快，那我為什麼還要學 TreeSet？

當你的情境符合以下三個條件時，TreeSet 就是你的不二之選：
1. 資料量適中：TreeSet 的底層是紅黑樹，每次新增、刪除、尋找的時間複雜度是 O(log n)
雖然比 HashSet 的 $O(1)$ 慢一點點
但在資料量適中時，這個效能差異微乎其微
2. 需要自動、即時排序：如果你用 ArrayList，每次加新資料都要手動呼叫 Collections.sort()，效能極差
而 TreeSet 是動態的，新資料一進來，內部立刻自動把它排到對的位置
3. 確保唯一性（不重複）：具備 Set 的天性，重複的資料絕對丟不進去

方案：當你同時需要 「排序」 又需要 「不重複」 時
TreeSet 是超越 ArrayList 與 HashSet 的方案